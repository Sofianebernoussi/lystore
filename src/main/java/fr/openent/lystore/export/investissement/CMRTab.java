...
public class CMRTab extends Investissement {
    ...
    public CMRTab(Workbook wb, JsonObject instruction) {
        ...
        query = "WITH values AS (    " +
                ...
                "  AND oce.override_region = false    AND oce.id_structure IN (    SELECT id    FROM lystore.specific_structures    WHERE type='" + CMR + "' ) " +
                ...
                "  WHERE instruction.id = ?    " +
                "  AND structure_program_action.structure_type =' " + CMR + "'   " +
                "  AND ore.id_structure IN (    SELECT id    FROM lystore.specific_structures    WHERE type='" + CMR + "'  ) " +
