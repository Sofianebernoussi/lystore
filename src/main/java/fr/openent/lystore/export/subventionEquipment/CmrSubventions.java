...
public class CmrSubventions extends TabHelper {
    private StructureService structureService;
    private ArrayList<Integer> codes = new ArrayList<>();
    private int arraylength = 5;
    private int lineNumber = 1;

    public CmrSubventions(Workbook workbook, JsonObject instruction) {
        super(workbook, instruction, "ANN. 1 RAPPORT CMR subventions");
...
//        });
    ...
    private void header(){
        JsonArray contentsHeader = new JsonArray()
                .add("ANNEXE au rapport")
                .add("Montant total dotations financières au titre du présent rapport")
                .add(Double.toString(545165120.16));

        CellRangeAddress merge = new CellRangeAddress(0, 0, 0, 4);
        for (int rowNumber = 0 ; rowNumber<contentsHeader.size() ; rowNumber++){
            sheet.addMergedRegion(merge);
            excel.setRegionHeader(merge, sheet);
            if(contentsHeader.getValue(rowNumber) instanceof String ){
                excel.insertBlackTitleHeader(0, rowNumber, contentsHeader.getString(rowNumber));
            } else {
                excel.insertBlackTitleHeader(0, rowNumber, "");
            }
        }
    }
